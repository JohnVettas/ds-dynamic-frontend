...
async function examOptions() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Failed to fetch semester's exams");
    ...
    try {
        const septResponse = await fetch("http://localhost:8000/jsonData/september_exams.json", {
            method: "HEAD"
        });
        if (septResponse.ok) isSeptember = true;
        else isSeptember = false;
    } catch (error) {
...
filterBtn.addEventListener("click", async function() {
    ...
    if (filterOn && !search) {
        ...
        teachersArray.forEach((teacher1)=>{
            const option = document.createElement("option");
            option.value = teacher1;
            option.textContent = teacher1;
            teacherSelect.appendChild(option);
        });
        //append each room to the box
        roomsArray.forEach((room1)=>{
            const option = document.createElement("option");
            option.value = room1;
            option.textContent = room1;
...
},{"24797d619a5dbbe3":"eR1sH","ecec44d163df7ab7":"1wC1H"}],"eR1sH":[function(require,module,exports,__globalThis) {
